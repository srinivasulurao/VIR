...
export class MemberHomePage {
  ...
  constructor(public navCtrl: NavController,
              ...
              public loader: LoadingController,public toastCtrl: ToastController,
  ...
  checkConnection() {
    this.platform.ready().then(() => {
      let networkStateDownlink = navigator.connection.downlink;
      let networkStateRtt = navigator.connection.rtt;
      console.log(navigator.connection);
      if(networkStateDownlink == 10 || networkStateRtt ==0){
        this.status=false;
      }else{
        this.status=true;
      }
  ...
  getOfflineStoresList(){
    ...
    alert.present(); // We will just show one alert, if the webservice fails.
  }); //Ajax Call ends here.
